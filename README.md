...
![AI-QCell 교대 품질 분석 센터](docs/images/quality_analytics.png)

`품질 분석 리포트` 모듈은 생산 검사 이력을 교대 단위 의사결정 자료로 변환합니다.

- 운영 대시보드 세션 또는 재현 가능한 교대 시뮬레이션 데이터 선택
- Lot·공정 구간 필터와 5개 품질 KPI 동기화
- 소그룹별 p-chart, 3σ 관리한계와 공정 이탈 신호 탐지
- 결함 유형별 Pareto 및 누적 기여도 분석
